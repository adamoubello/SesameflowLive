 ...
 * 		- modification de la fonction format_duree_tache() en format_duree_workflow
 *      - ajout dynamique de l'unité de la durée d'un workflow
 *      - ajout du typedoc sur l'hyperlien du libellé document dans la grille  
 */
    
	$btn_nouveau = "";
    //print_r($listeworkflow);
	if(! is_null($listeworkflow))
	{	
	...
	$nbr_workflow = count($listeworkflow);
	if ($nbr_workflow > 0)
	{
		...
		if ($limit > 0)
		{
			...
			else 
			{
				...
				//l'affichage des colonnes de la grille des tâches va varier suivant la page web en cours
				        	$ldata_grid_workflow->addColumn(new Structures_DataGrid_Column(ucfirst($translate["numworkflow"]), 'numworkflow', 'numworkflow' , array("align"=>"center" , "width" => "10%") , null , 'format_numworkflow()'));
						$ldata_grid_workflow->addColumn(new Structures_DataGrid_Column(ucfirst($translate["circuit"]), 'libcircuit', 'libcircuit' , array("align"=>"center" , "width" => "10%") , null , 'format_circuit_workflow()'));
						...
						$ldata_grid_workflow->addColumn(new Structures_DataGrid_Column(ucfirst($translate["tache"]), 'libtache', 'libtache', array("align"=>"center" , "width" => "20%") , null , 'format_libtache()'));			
						...
						$ldata_grid_workflow->addColumn(new Structures_DataGrid_Column(ucfirst($translate["options"]), null, null, array("align"=>"center" , "width" => "10%") , null , 'format_option_workflow()' ));	
		...
		//	    'class' => 'listResultsRowHover'
				...
				if ($count > 0){
					...
					if (PEAR::isError($test)) {
						...
						$result_recherche_workflow = $test->getMessage();
				...
				$result_recherche_workflow = $btn_nouveau;
				 
				$result_recherche_workflow .= "<center>{$title}</center><br/><center>{$lpager}</center>";
				
				$result_recherche_workflow .= $table->toHtml();
		...
		else
		{
			...
			$result_recherche_workflow .= ucfirst($translate["any_workflow_found"]);
