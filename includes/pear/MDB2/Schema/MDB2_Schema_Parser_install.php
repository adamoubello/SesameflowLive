 ...
 * Parser adapté à UltraLog pour la gestion des fichiers volumineux : le parser par défaut stocke toutes les informations du fichier XML lu en mémoire. Ce qui finit par entrainer un dépassement de mémoire. La solution consiste donc à créer les tables au fur et à mesure de la lecture puis de les supprimer de la mémoire. Le parser original était indépendant du Schema et s'arretait à la moindre erreur. Ce parser est dédié exclusivement à l'installation d'une base (ou d'autres actions similaires nécessitant l'ouverture de fichiers volumineux).
 * @package gestionBDD
 * @author Rémi FRESKO
 * @copyright Yob S.A
 */
class MDB2_Schema_Parser_install extends MDB2_Schema_Parser
{
	/**
	 * Le schéma de la base à laquelle il est rattachée.
	 * @var MDB2_Schema
	 */
	private $schema_bdd;
	/**
	 * Tableau associatif permettant d'avoir un retour sur les erreurs ou succès.
	 * @var array
	 */
	public $adverts;
	
	
	function __construct($variables, $fail_on_invalid_names = true, $structure = false, $valid_types = array(), $force_defaults = true, &$schema_bdd)
	{
        ...
        $this->schema_bdd = &$schema_bdd;
        $this->adverts = array("success"=>array(), "error"=>array());
    }

    function MDB2_Schema_Parser_redef($variables, $fail_on_invalid_names = true, $structure = false, $valid_types = array(), $force_defaults = true, &$schema_bdd)
    {
	    $this->__construct($variables, $fail_on_invalid_names, $structure, $valid_types, $force_defaults, $schema_bdd);	    
	...
	function cdataHandler($xp, $data)
	{
	        ...
	        case 'database-table-initialization-update-field-value':
	        	...
	        	$count = count($this->init['data']['field'])-1;
	        	
	        	if(!empty($this->init['data']['field'][$count]['group']['data']))
	        	{
	        		$this->init['data']['field'][$count]['group']['data'] .= $data;
	        	}
	        	else 
	        	{
	        		$this->init['data']['field'][$count]['group']['data'] = $data;
	...
	/**
	 * Cette méthode est appellée dès qu'on rencontre une balise fermante	 
	 */
    function endHandler($xp, $element)
    {
        ...
        /* Table definition */
        /**
         * Modification principale apportée à la classe Parser. On crée la table puis on l'efface de la data_definition
         */
        case 'database-table':
            ...
            if (PEAR::isError($result)) {
                ...
                //var_dump($this);
                //var_dump($this->table);
                $result = $this->schema_bdd->createTable($this->table_name, $this->table);
                
                if (PEAR::isError($result)) 
                {   
                	array_push($this->adverts["error"], $this->table_name." : ".$result->getMessage()."<br/>".$result->getUserInfo()."<br/>");
            	}
            	else 
            	{
            		array_push($this->adverts["success"], $this->table_name);
            	}
            	unset($this->table);            	
            	
            	/*$fp_test=fopen($GLOBALS['DOCUMENT_ROOT'].'/temp/gestionBDD/test_memoire_parser.txt', 'a');
            	fwrite($fp_test, $this->table_name." ".memory_get_usage()."\n");
            	fclose($fp_test);*/
                //echo $this->table_name."<br/>";
                
               // string $table_name, array $table, [bool $overwrite = false]
            }
            break;
        case 'database-table-name':
